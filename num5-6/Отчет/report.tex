...
\par
Для всех ответов языковой (при $\lambda = 0.5$) и векторной модели (см. лаб. № 3) первым 15-ти документам была проставлена оценка релевантности по шкале от 0 до 2. Так же были составлены идеальные варианты потенциальной выдачи для обеих моделей для последующей оценки NDCG. Сами разметки можно увидеть по ссылке \url{https://github.com/legion15q/sem2/tree/master/num5-6/py}.
Благодаря этому была построена оценка качества поисковой выдачи, полный ход вычислений см. по ссылке \url{https://github.com/legion15q/sem2/blob/master/num5-6/%D0%9E%D1%82%D1%87%D0%B5%D1%82/aXl9lWSR5qE.jpg}:\\
\subsection{Языковая модель}
$$
DCG = \sum_1^{15} \dfrac{rel_r}{log_2(r+1)},
$$
$$
DCG_1 = 2.98,
$$
...
NDCG_{average} = 0.647.
$$
В итоге, в данной реализации получилось, что в среднем языковая модель показала себя лучше.
\section{Приложение}
\begin{verbatim}
import pymorphy2
